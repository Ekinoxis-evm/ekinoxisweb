...
import type { ProductWithRelations, HackathonWithStatus, SubLink } from '@/lib/supabase/types';
...
function getSubLinks(p: ProductWithRelations): SubLink[] {
...
function SubLinksBlock({ links, label }: { links: SubLink[]; label: string }) {
...
function ProductCard({ product, language, modulesLabel }: {
...
function SectionHeader({ uid, title, description }: { uid: string; title: string; description: string }) {
  return (
    <motion.div
    ...
    >
      <motion.div variants={fadeInUp} className="flex items-center gap-3 font-mono text-[10px] text-primary/60 tracking-widest uppercase mb-4">
        ...
        <span className="border border-primary/30 px-2 py-1">{uid}</span>
      </motion.div>
      <motion.div variants={fadeInUp} className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        <h2 className="font-headline text-4xl md:text-6xl font-bold tracking-tighter leading-[0.9] text-on-surface">
          {title}
        </h2>
...
export default function ProductsClient({ products, hackathons }: Props) {
  ...
  const inHouse = products.filter((p) => (p.project_type ?? 'product') === 'product');
  const clients = products.filter((p) => p.project_type === 'client');
  const experiments = products.filter((p) => p.project_type === 'experiment');

  const [featured, ...rest] = inHouse;
    ...
    <AnimatedBackground variant="ambient">
      <div className="min-h-screen py-24 px-6">
        <div className="max-w-screen-2xl mx-auto">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
                ...
                <div className="p-12 flex flex-col justify-between">
                  <div>
                    ...
                    <SubLinksBlock links={featuredSubLinks} label={t.modulesLabel} />
                    {featured.hackathon && (
          ...
          >
            {rest.map((product) => (
              <ProductCard key={product.id} product={product} language={language} modulesLabel={t.modulesLabel} />
            ))}
          </motion.div>
          ...
          {clients.length > 0 && (
            <>
              <SectionHeader uid="CLIENT_WORK" title={t.clientWork.title} description={t.clientWork.description} />
              <motion.div
                variants={staggerContainer}
                initial="hidden"
                whileInView="visible"
          ...
          {experiments.length > 0 && (
            <>
              <SectionHeader uid="EXPERIMENTS" title={t.experiments.title} description={t.experiments.description} />
              <motion.div
                variants={staggerContainer}
                initial="hidden"
                whileInView="visible"
          ...
          >
            <span>PRODUCT_COUNT: {inHouse.length} / CLIENTS: {clients.length} / EXPERIMENTS: {experiments.length}</span>
